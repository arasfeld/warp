...
export const Chip = forwardRef<HTMLInputElement, ChipProps>(
  (
    ...
    const getVariantClasses = () => {
      const baseClasses =
        'inline-flex items-center justify-center cursor-pointer transition-all duration-200 border-2 font-medium';

      if (variant === 'filled') {
        // Color mapping for filled variant
        const colorMap: Record<
          string,
          { bg: string; text: string; border: string; checked: string }
        > = {
          primary: {
            bg: 'bg-primary',
            text: 'text-white',
      ...
      if (variant === 'outline') {
        ...
        const colorConfig = colorMap[color] || {
          ...
          unchecked:
            'bg-transparent text-muted-foreground border-gray-300 dark:border-gray-600',
        };
      ...
      if (variant === 'light') {
        ...
        const colorConfig = colorMap[color] || {
          ...
          unchecked:
            'bg-gray-100 dark:bg-gray-800 text-muted-foreground border-transparent',
        };
